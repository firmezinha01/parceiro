      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2 sm:gap-2.5">
            ...
            <button
              onClick={() => {
                if (confirm('Deseja limpar todos os dados cadastrados e pedidos para iniciar uma base vazia?')) {
                  resetAllData();
                }
              }}
            ...
            {/* Usuário Autenticado & Botão Sair */}
            {authUser && (
              <div className="flex items-center gap-1.5 pl-1.5 border-l border-slate-700/80">
                <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-800 border border-slate-700 text-xs">
                  {authUser.provider === 'google' ? (
                    ...
                    <div className="w-4 h-4 rounded-full bg-amber-500/20 text-amber-400 border border-amber-400/40 flex items-center justify-center shrink-0">
                      <User className="w-2.5 h-2.5" />
                    </div>
                  )}
                  <span className="text-white font-bold text-xs max-w-[110px] truncate hidden sm:inline">
                    {authUser.name}
                  </span>
                </div>
                ...
                <button
                  ...
                  className="p-2 rounded-xl text-red-400 hover:text-white bg-slate-800 hover:bg-red-600/80 border border-slate-700 transition cursor-pointer"
                >
